...
def show_exam_result(request, course_id, submission_id):
    ...
    choice = submission.choices.values()
    

    context['course'] = course
    context['questions']=[]
    context['selected_choices']=submission.choices.all()
    ...
    for question in Question.objects.filter(course=course):
        questionMp = {}
        questionMp["question"]=question
        questionMp["choices"]=Choice.objects.filter(question=question)
        context['questions'].append(questionMp)

        totalGrade += question.grade
        if question.is_get_score(submitted_anwsers) :
            grade += question.grade
    
    context['grade'] = grade
    context['totalGrade'] = totalGrade
    context['gradeRate'] = int(grade/totalGrade * 100)
